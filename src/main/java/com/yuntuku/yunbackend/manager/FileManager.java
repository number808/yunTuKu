...
public class FileManager {
     ...
     * @param uploadPathPrefix 上传路径前缀,/uploadPathPrefix/文件名
     * @return
     */
    ...
    public UploadPictureResult uploadPictureByUrl(String fileUrl, String uploadPathPrefix) {
        ThrowUtils.throwIf(fileUrl.isEmpty(), ErrorCode.NOT_FOUND_ERROR);

        // 图片上传地址

        String uuid = RandomUtil.randomString(16); // 随机字符串
        String originFilename = FileUtil.mainName(fileUrl); // 原始文件名
        String uploadFilename = String.format("%s_%s.%s",
                DateUtil.formatDate(new Date()),  // 日期
                uuid,                            // 唯一串
                FileUtil.getSuffix(originFilename) // 后缀（jpg/png）
        );
        String uploadPath = String.format("/%s/%s", uploadPathPrefix, uploadFilename);
        File file = null;
        try {
